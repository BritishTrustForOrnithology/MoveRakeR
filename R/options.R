#' Global options for printing \code{Track} datasets
#'
#' @description
...
#' options.
#'
#' @param Print_method One of either \emph{"Track"} or \emph{"data.frame"} to determine printing as for \code{Track} family
...
#' @param Print_type One of either \emph{"subset"} or \emph{"all"} controlling the extent of data
...
#'
#' @export
reset_options_mR <- function(){
  get_options <<- options_mR$new()
}

#' @noRd
#' @export
options_mR <- R6::R6Class("options_mR", list(
  print_method = "Track",
  print_type = "subset",
  options = function(Print_method = "Track", Print_type = "subset"){
...
#' @rdname reset_options_mR
#' @export
set_options_mR <- function(Print_method = "Track", Print_type = "subset"){

  variables <- ls()
  locEnv <- new.env()
  for(i in 1:length(variables)) {
    assign(variables[i],get(variables[i]),envir=locEnv)
  }
  dum <- list()
  for(i in 1:length(variables)) {
    dum[[i]] <-  get(variables[i],envir=locEnv)
  }
  names(dum) <- variables

  if(dum$Print_method != "Track" & dum$Print_method != "data.frame"){
    stop("Only Track and data.frame options implemented")
  }
  if(dum$Print_type != "all" & dum$Print_type != "subset"){
    stop("Only all and subset types implemented")
  }

  get_options$options(Print_method=dum$Print_method, Print_type = dum$Print_type)

}

#' @rdname reset_options_mR
#' @export
get_options_mR <- function(){
  out <- NULL
  attributes(out)$print_method <-  get_options$print_method
  attributes(out)$print_type <-  get_options$print_type
  return(out)
